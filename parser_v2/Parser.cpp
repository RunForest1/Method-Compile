...
void Parser::initMatrix()
{
    ...
    auto IF = (LexemType)1001;    auto THEN = (LexemType)1002;  auto ELSE = (LexemType)1003;
    auto WHILE = (LexemType)1004; auto DO = (LexemType)1005;    auto READ = (LexemType)1006;
    auto WRITE = (LexemType)1007; auto SIN = (LexemType)1008;   auto COS = (LexemType)1009;
    auto EXP = (LexemType)1010;   auto POW = (LexemType)1011;   auto LOG = (LexemType)1012;

    auto LPAR = (LexemType)2001;   auto RPAR = (LexemType)2002;
    auto LBRACK = (LexemType)2003; auto RBRACK = (LexemType)2004;
    auto SEMI = (LexemType)2005;   auto COMMA = (LexemType)2006;
    auto LBRACE = (LexemType)2007; auto RBRACE = (LexemType)2008;

    // Список токенов, с которых может начинаться Factor (для инициализации Expression/Term)
    std::vector<LexemType> firstFactor = {
        ID, INT, FLOAT, STRING, LPAR, ADD_OP, SIN, COS, EXP, POW, LOG
    };
    ...
    for (auto t : {TERM, ELSE, RBRACE, SEMI}) M[NonTerm::Program][t] = {};
    ...
    for (auto t : {ID, IF, WHILE, READ, WRITE, TERM, RBRACE, SEMI}) M[NonTerm::ElsePart][t] = {};
    ...
    for (auto t : {SEMI, THEN, DO, RPAR, RBRACK, COMMA, COMP_OP, ELSE}) M[NonTerm::ExpressionTail][t] = {};
    ...
    for (auto t : {ADD_OP, SEMI, THEN, DO, RPAR, RBRACK, COMMA, COMP_OP, ELSE}) M[NonTerm::TermTail][t] = {};
    ...
    // Функции в Factor
    for (auto f : {SIN, COS, EXP, POW, LOG}) {
    ...
    for (auto t : {ASSIGN, SEMI, ADD_OP, MULT_OP, COMP_OP, RPAR, COMMA, THEN, DO, ELSE, RBRACK}) M[NonTerm::ArrayIndex][t] = {};
    ...
    for (auto f : {std::make_pair(SIN, "SIN"), std::make_pair(COS, "COS"), std::make_pair(EXP, "EXP"), std::make_pair(LOG, "LOG")}) {
